...
public class BaseGridRendererHelper {
    ...
    private final IBaseGridWidget view;

    public BaseGridRendererHelper( final IGridData model,
                                   final IBaseGridWidget view ) {
     ...
     * @return
     */
    public double getColumnOffset( final IGridColumn<?> column ) {
    ...
    public double getColumnOffset( final int columnIndex ) {
        ...
        final List<IGridColumn<?>> columns = model.getColumns();
     ...
     * @return
     */
    public double getColumnOffset( final List<IGridColumn<?>> columns,
                                   final int columnIndex ) {
        double columnOffset = 0;
        for ( int idx = 0; idx < columnIndex; idx++ ) {
            final IGridColumn<?> column = columns.get( idx );
            if ( column.isVisible() ) {
                columnOffset = columnOffset + column.getWidth();
            }
        }
     ...
     * @return
     */
    public double getRowOffset( final IGridRow row ) {
    ...
    public double getRowOffset( final int rowIndex ) {
        ...
        for ( int i = 0; i < rowIndex; i++ ) {
            final IGridRow row = model.getRow( i );
     ...
     * @return
     */
    public double getWidth( final List<IGridColumn<?>> columns ) {
        double width = 0;
    ...
    public RenderingInformation getRenderingInformation() {
        final IBounds bounds = getVisibleBounds();
        final List<IGridColumn<?>> allColumns = new ArrayList<IGridColumn<?>>();
        final List<IGridColumn<?>> bodyColumns = new ArrayList<IGridColumn<?>>();
        final List<IGridColumn<?>> floatingColumns = new ArrayList<IGridColumn<?>>();
     ...
     * @return
     */
    private IBounds getVisibleBounds() {
        final IGridLayer gridLayer = ( (GridLayer) view.getLayer() );
        final IBounds bounds = gridLayer.getVisibleBounds();
        return bounds;
    }
    ...
    public static class ColumnInformation {
        ...
        ColumnInformation( final IGridColumn<?> column,
    ...
    public static class RenderingInformation {
        ...
        RenderingInformation( final IBounds bounds,
                              final List<IGridColumn<?>> allColumns,
    ...
    public static class RenderingBlockInformation {
        ...
        RenderingBlockInformation( final List<IGridColumn<?>> columns,
                                   final double x,
