...
import org.uberfire.ext.wires.core.grids.client.widget.pinning.GridRestriction;
...
public class BaseGridWidgetMouseDoubleClickHandler implements NodeMouseDoubleClickHandler {
    ...
    protected IGridPinnedModeManager pinnedModeManager;
        ...
        if ( !pinnedModeManager.isGridPinned() ) {
            ...
            pinnedModeManager.exitPinnedMode( new Command() {
                ...
                public void execute() {
                    for ( IMediator mediator : getMediators() ) {
                        if ( mediator instanceof RestrictedMousePanMediator ) {
                            ( (RestrictedMousePanMediator) mediator ).setRestriction( pinnedModeManager.getDefaultRestriction() );
                        }
                    }
                }
            } );
    ...
    private IBounds getBounds( final IBaseGridWidget gridWidget ) {
        return new Bounds( gridWidget.getX(),
                           gridWidget.getY(),
                           gridWidget.getWidth(),
