 ...
 */
public class BaseGridWidget extends Group implements IBaseGridWidget,
                                                     NodeMouseClickHandler {

    protected final ISelectionsTransformer bodyTransformer;
    protected final ISelectionsTransformer floatingColumnsTransformer;
    ...
    protected final List<IGridColumn<?>> allColumns = new ArrayList<IGridColumn<?>>();
    protected final List<IGridColumn<?>> bodyColumns = new ArrayList<IGridColumn<?>>();
    protected final List<IGridColumn<?>> floatingColumns = new ArrayList<IGridColumn<?>>();

    protected IGridData model;
    ...
    public BaseGridWidget( final IGridData model,
                           final IGridSelectionManager selectionManager,
                           final IGridPinnedModeManager pinnedModeManager,
    ...
    @Override
    public void setRenderer( final IGridRenderer renderer ) {
        this.renderer = renderer;
    }
    ...
    protected void drawWithoutTransforms( Context2D context,
        ...
        final List<IGridColumn<?>> allColumns = renderingInformation.getAllColumns();
    ...
    public Group setVisible( final boolean visible ) {
        if ( !visible ) {
            for ( IGridColumn<?> gc : getModel().getColumns() ) {
                if ( gc instanceof IHasMultipleDOMElementResources ) {
                    ( (IHasMultipleDOMElementResources) gc ).destroyResources();
                }
            }
        }
    ...
    protected void drawHeader( final BaseGridRendererHelper.RenderingInformation renderingInformation,
        ...
        final IBounds bounds = renderingInformation.getBounds();
        final List<IGridColumn<?>> allColumns = renderingInformation.getAllColumns();
        final BaseGridRendererHelper.RenderingBlockInformation bodyBlockInformation = renderingInformation.getBodyBlockInformation();
        final BaseGridRendererHelper.RenderingBlockInformation floatingBlockInformation = renderingInformation.getFloatingBlockInformation();
        final List<IGridColumn<?>> bodyColumns = bodyBlockInformation.getColumns();
    ...
    protected void drawBody( final BaseGridRendererHelper.RenderingInformation renderingInformation,
        ...
        if ( !isSelectionLayer ) {
            for ( IGridColumn<?> column : model.getColumns() ) {
                if ( bodyColumns.contains( column ) || floatingColumns.contains( column ) ) {
                    if ( column.getColumnRenderer() instanceof IHasMultipleDOMElementResources ) {
                        ( (IHasMultipleDOMElementResources) column.getColumnRenderer() ).initialiseResources();
                    }
                } else if ( column instanceof IHasDOMElementResources ) {
                    ( (IHasDOMElementResources) column.getColumnRenderer() ).destroyResources();
                }
            }
        }
        ...
        if ( !isSelectionLayer ) {
            for ( IGridColumn<?> column : bodyColumns ) {
                if ( column.getColumnRenderer() instanceof IHasMultipleDOMElementResources ) {
                    ( (IHasMultipleDOMElementResources) column.getColumnRenderer() ).freeUnusedResources();
                }
            }
            for ( IGridColumn<?> column : floatingColumns ) {
                if ( column.getColumnRenderer() instanceof IHasMultipleDOMElementResources ) {
                    ( (IHasMultipleDOMElementResources) column.getColumnRenderer() ).freeUnusedResources();
                }
            }
     ...
     */
    protected Group renderGridHeaderWidget( final List<IGridColumn<?>> allColumns,
     ...
     */
    protected Group renderGridBodyWidget( final List<IGridColumn<?>> blockColumns,
     ...
     * @return
     */
    protected Group renderSelectedRanges( final List<IGridColumn<?>> blockColumns,
                                          final double absoluteColumnOffsetX,
                                          final int minVisibleRowIndex,
                                          final int maxVisibleRowIndex,
                                          final ISelectionsTransformer transformer,
